...
router.post("/", (req, res) => {
  ...
  con.beginTransaction(function (err) {
    ...
    con.query(
      sql,
      [customerId, dateShipped, datePlaced, req.body.status, req.body.shipping],
      function (error, results, fields) {
        ...
        console.log(results, req.body.items);

        req.body.items.map((item) => {
          let sql2 = `INSERT INTO orderline (orderinfo_id, item_id, quantity) VALUES(?, ?, ?)`;
          con.query(
            sql2,
            [orderinfoId, item.item_id, item.quantity],
            function (error, results, fields) {
              if (error) {
                return con.rollback(function () {
                  throw error;
              ...
              con.query(
                ...
                function (error, results, fields) {
                  ...
                  con.commit(function (err) {
                    ...
                    return res
                      .status(200)
                      .json({ success: true, message: "order success" });
                  });
                }
              );
            }

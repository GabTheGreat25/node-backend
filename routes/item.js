...
router.post("/", uploadOptions.single("uploads"), (req, res) => {
  ...
  con.query(
    sql,
    [req.body.description, req.body.sell_price, req.body.cost_price, imagePath],
...
router.put("/:id", uploadOptions.single("uploads"), (req, res) => {
  ...
  const basePath = `${req.protocol}://${req.get("host")}/public/uploads/`;
  let imagePath = `${basePath}${fileName}`;
  let sql = `UPDATE item SET description = ?, sell_price = ?, cost_price = ?, imagePath = ? WHERE item_id = ${req.params.id}`;
  console.log(fileName, sql);
  con.query(
    sql,
    [
